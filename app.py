...
def run_powershell_json(script: str) -> Any:
    ...
    The function now uses a longer timeout (10 seconds) and handles
    ``subprocess.TimeoutExpired`` gracefully, returning ``None`` when the
    script does not finish in time. This prevents the publisher from
    crashing if the hardware monitor is unavailable or slow.
    """
    try:
        ...
        LOGGER.warning("PowerShell script timed out after 10 seconds")
        return None
